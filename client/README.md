...
The menu can configure or reconfigure credentials, run DDNS or certificate synchronization immediately, install cron or systemd schedules, show status and diagnostics, and uninstall. Non-interactive commands are also available:
...
Set a custom panel endpoint, schedule, certificate directory, and certificate-change reload command during configuration. For scripted provisioning, export `DDNS_ENDPOINT`, `DDNS_API_KEY`, `CERT_ENDPOINT`, `CERT_TOKEN`, `CERT_DIR`, and `RELOAD_CMD` before running `configure`.
